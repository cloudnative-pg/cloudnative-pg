	...
	Context("automatic pause/resume during switchover", Ordered, func() {
		...
		It("pauses pooler during switchover and resumes after", func() {
			...
			By("creating a pooler with pauseDuringSwitchover enabled", func() {
				pgbouncerasserts.AssertPgBouncerPoolerIsSetUp(env, namespace, poolerSampleFile, 1)
			})
			...
			By("verifying the pooler was paused during the switchover", func() {
				// The pause is already over when AssertSwitchover returns, so use the
				// PoolerPaused event, which persists, to prove that it happened.
				Eventually(func(g Gomega) {
					eventList := corev1.EventList{}
					err := env.Client.List(env.Ctx, &eventList,
						ctrlclient.InNamespace(namespace),
						ctrlclient.MatchingFields{
							"involvedObject.kind": "Pooler",
							"involvedObject.name": poolerName,
						})
					...
					g.Expect(reasons).To(ContainElement("PoolerPaused"))
				}, 60).Should(Succeed())
			})
