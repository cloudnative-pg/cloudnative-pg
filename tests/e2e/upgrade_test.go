...
var _ = Describe("Upgrade", Label(tests.LabelUpgrade, tests.LabelNoOpenshift), Ordered, Serial, func() {
	...
	BeforeEach(func() {
		...
		err := namespace.EnsureNamespace(env.Ctx, env.Client, operatorNamespace)
	...
	cleanupOperatorAndMinio := func() error {
		...
		if CurrentSpecReport().Failed() {
			...
			namespace.DumpNamespaceObjects(
				env.Ctx, env.Client,
		...
		if err := namespace.DeleteNamespaceAndWait(env.Ctx, env.Client, operatorNamespace, 60); err != nil {
