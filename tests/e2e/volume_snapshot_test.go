		...
		Context("Can restore from a Volume Snapshot", Ordered, func() {
			...
			It("correctly executes PITR with a cold snapshot", func() {
				...
				By("creating the cluster to be restored through snapshot and PITR", func() {
					clusterasserts.AssertCreateCluster(env, testTimeouts, namespace, clusterToRestoreName,
						clusterSnapshotRestoreFile)
					clusterasserts.AssertClusterIsReady(env, namespace, clusterToRestoreName,
