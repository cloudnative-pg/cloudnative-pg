...
var _ = Describe("Cluster Hibernation with plugin", func() {
	...
	Context("hibernate on", func() {
		...
		verifyClusterResources := func(namespace, clusterName string, role utils.PVCRole) {
			By(fmt.Sprintf("verifying cluster resources are removed "+
				"post hibernation where role %v", role), func() {
				...
				By(fmt.Sprintf("verifying cluster %v PVCs are removed", clusterName), func() {
					...
					Expect(len(pvcList.Items)).Should(BeEquivalentTo(expectedPvcCount))
				})
		...
		When("cluster setup with PG-WAL volume", func() {
			It("hibernation process should work", func() {
				...
				verifyClusterResources(namespace, clusterName, utils.PVCRolePgWal)
				verifyClusterResources(namespace, clusterName, utils.PVCRolePgData)
