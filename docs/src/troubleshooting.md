...
If the instance that bootstraps your Cluster crashes with a "Bus error (core
...
### Bootstrap hangs in running status

If the first instance of your Cluster hangs while bootstrapping, with the
message:
...
An instance needs to access the Kubernetes API while bootstrapping its data
directory. Please check your networking.
