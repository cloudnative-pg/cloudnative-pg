...
`ENABLE_WEBHOOK_NAMESPACE_SUFFIX` | When set to `true`, the operator looks up its `MutatingWebhookConfiguration` and `ValidatingWebhookConfiguration` under names suffixed with `-<OPERATOR_NAMESPACE>` (e.g. `cnpg-mutating-webhook-configuration-cnpg-team-a` and `cnpg-validating-webhook-configuration-cnpg-team-a`) instead of the default unsuffixed names. Use this when running multiple namespaced operator instances on the same cluster to avoid webhook configuration name collisions. The flag only changes the names the operator looks up; it does not create or rename any resource. CloudNativePG does not ship manifests for the suffixed configurations — when this flag is enabled you are responsible for creating and maintaining the `MutatingWebhookConfiguration` and `ValidatingWebhookConfiguration` under the matching suffixed names. If they are missing, the operator's webhook setup will fail. Default is `false`.
