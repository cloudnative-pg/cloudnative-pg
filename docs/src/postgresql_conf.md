...
If you're familiar with PostgreSQL, you're aware of the following two files
for configuring an instance:

- `postgresql.conf` – Main runtime configuration file of PostgreSQL.
- `pg_hba.conf` – Client's authentication file.

Due to the concepts of declarative configuration and immutability of the PostgreSQL
containers, you aren't allowed to directly touch those files. Configuration
is possible through the `postgresql` section of the `Cluster` resource definition.
There, you define custom `postgresql.conf` and `pg_hba.conf` settings by way of the
`parameters` and the `pg_hba` keys.

These settings are the same across all instances.

!!! Warning
    Don't use the `ALTER SYSTEM` query to change the configuration of
    the PostgreSQL instances in an imperative way. Changing some of the options
    that are normally controlled by the operator might lead to an
    unpredictable/unrecoverable state of the cluster.
    Moreover, `ALTER SYSTEM` changes aren't replicated across the cluster.
...
These settings are automatically added:
...
The `custom.conf` file contains user-defined settings in the
...
!!! Seealso "PostgreSQL GUCs: Grand Unified Configuration"
    ...
    also known as Grand Unified Configuration (GUC).
    CloudNativePG accepts only strings for the PostgreSQL parameters.

The content of `custom.conf` is generated and maintained by the
...
!!! Warning
    You must plan for WAL segments retention in your PostgreSQL
    ...
    `replicationSlots.highAvailability` option. (For more information, see
    [Replication](replication.md#replication-slots-for-high-availability).)

    Without replication slots or continuous backups in place, configuring
    `wal_keep_size` or `wal_keep_segments` is the only way to
    protect standbys from falling out of sync.
    If a standby does fall out of sync, it produces error
    messages like:
    `"could not receive data from WAL stream: ERROR: requested WAL segment ************************ has already been removed"`.
    This condition requires you to dedicate a part of your `PGDATA`, or the volume
...
Since the fixed parameters are added at the end, you can't override them
by way of the YAML configuration. Those parameters are required for correct WAL
archiving and replication.

### Replication settings

The `primary_conninfo`, `restore_command`,  and `recovery_target_timeline`
parameters are managed by the operator according to the state of
...
instance manager parses it and outputs it in JSON format.
For this reason, all log settings in PostgreSQL are fixed and can't be
changed.

For more information, see [Logging](logging.md).

### Shared preload libraries

The `shared_preload_libraries` option in PostgreSQL exists to specify one or
more shared libraries to be preloaded at server start, in the form of a
comma-separated list. Typically, it's used in PostgreSQL to load those
...
!!! Important
    If a specified library isn't found, the server fails to start,
    preventing CloudNativePG from any self-healing attempt. This condition requires
    manual intervention. Make sure you always test both the extensions and
    the settings of `shared_preload_libraries` if you plan to directly manage its
    content.

CloudNativePG can manage the content of the
`shared_preload_libraries` option for some of the most used PostgreSQL
extensions. See [Managed extensions](#managed-extensions)
for details.

Specifically, as soon as the operator notices that a configuration parameter
requires one of the managed libraries, it adds the needed
library. The operator also removes the library as soon as no actual parameter
requires it.

!!! Important
    Keep in mind that removing libraries from
    `shared_preload_libraries` requires a restart of all instances in the cluster
    to be effective.

You can provide additional `shared_preload_libraries` as a list of strings by way of
`.spec.postgresql.shared_preload_libraries`. The operator
merges them with the ones that it manages.
...
using them, normally views or functions managed by way of the `CREATE EXTENSION`
command to be run in a database. (The `DROP EXTENSION` command typically removes
those objects.)

For such libraries, CloudNativePG handles the creation
...
!!! Note
    This query also includes template databases like `template1`.
...
without having to manually run `EXPLAIN` (helpful for tracking
down unoptimized queries).

You can enable `auto_explain` by adding to the configuration a parameter
that starts with `auto_explain.` as in the following example excerpt. The example
logs execution plans of queries that take longer than 10 seconds
to complete.
...
!!! Note
    Enabling `auto_explain` can lead to performance issues. See [`the auto explain documentation`](https://www.postgresql.org/docs/current/auto-explain.html).
...
As explained previously, the operator adds
`pg_stat_statements` to `shared_preload_libraries` and runs `CREATE EXTENSION IF
NOT EXISTS pg_stat_statements` on each database. This enables you to run queries
against the `pg_stat_statements` view.

#### Enabling `pgaudit`

The `pgaudit` extension provides detailed session and object audit logging by way of the standard PostgreSQL logging facility.

CloudNativePG has transparent and native support for
[PGAudit](https://www.pgaudit.org/) on PostgreSQL clusters. For more information, see [PGAudit logs](logging.md#pgaudit-logs).
...
that starts with `pg_failover_slots.` As explained earlier, the operator
transparently manages the `pg_failover_slots` entry in the
`shared_preload_libraries` option depending on this parameter.

See [the `pg_failover_slots` documentation](https://www.enterprisedb.com/docs/pg_extensions/pg_failover_slots)
...
the default authentication method is `scram-sha-256` starting with this
PostgreSQL version.

PostgreSQL 13 and earlier versions use `md5` as the default authentication
method.

The resulting `pg_hba.conf` looks like this:
...
For more information on `pg_hba.conf`, see [the PostgreSQL documentation](https://www.postgresql.org/docs/current/auth-pg-hba-conf.html).
...
This example enables access for the app user to the `app`
database using MD5 password authentication (you can use `scram-sha-256`
if you prefer) by way of a secure channel (`hostssl`).

### LDAP configuration

Under the `postgres` section of the cluster spec is an optional `ldap` section. This section is available for defining an LDAP
configuration to convert into a rule added into the `pg_hba.conf` file.

Two modes are supported. `simple bind` mode requires specifying a `server`, `prefix`, and `suffix` in the LDAP 
section. `search+bind` mode requires specifying `server`, `baseDN`, `binDN`, and a `bindPassword`, which is
a secret containing the LDAP password. Additionally, in `search+bind` mode you have the option to specify a
`searchFilter` or `searchAttribute`. If no `searchAttribute` is specified, the default one of `uid` is used.

Additionally, both modes allow specifying a `scheme` for ldapscheme and a `port`. `scheme` and `port` are
optional.

This example shows the `search+bind` filled out:
...
After the change, the cluster instances immediately reload the
configuration to apply the changes.
If the change involves a parameter requiring a restart, the operator
...
configuration option. In CloudNativePG, we recommend limiting the values to
either of the following:

- `posix`, which relies on POSIX shared memory allocated using `shm_open` (default setting)
- `sysv`, which is based on System V shared memory allocated using `shmget`
...
The default setting of `posix` is enough in most cases, considering that
the operator mounts a memory-bound `EmptyDir` volume called `shm`
under `/dev/shm`. You can verify the size of this volume inside the running
...
If you want to set a maximum size for the `shm` volume, you can do so by
...
If your Kubernetes cluster has a high enough value for the `SHMMAX`
...
An alternative method is to run:
...
Some PostgreSQL configuration parameters must be managed exclusively by the
operator. The operator prevents the user from setting them using a webhook.

Users aren't allowed to set the following configuration parameters in the
`postgresql` section:
