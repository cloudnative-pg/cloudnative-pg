...
You can install the [latest operator manifest](https://raw.githubusercontent.com/cloudnative-pg/cloudnative-pg/release-1.19/releases/cnpg-1.19.0.yaml)
...
`cnpg-controller-manager` when installing via the manifest or the `cnpg`
plugin. When installing using Helm, the default name is `cnpg-cloudnative-pg`.
