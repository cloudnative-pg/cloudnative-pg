...
:   Annotation can be applied on a `Cluster` resource.
    ...
    created by the operator (for initialization, recovery, cloning, and join operations).
    ...
    **Note**: This annotation is validated when the cluster is created or updated.
    ...
    Example of patching termination grace period on the job pod template:
    ...
    metadata:
      ...
      annotations:
        cnpg.io/jobPatch: '[{"op": "add", "path": "/spec/template/spec/terminationGracePeriodSeconds", "value": 60}]'
    spec:
      # ...
    ```
