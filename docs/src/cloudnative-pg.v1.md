...
| `podUID` _string_ | PodUID is the UID of the Pod that was evaluated when the issue was<br />confirmed. Used to detect that the instance has since been rebuilt<br />(for example with `kubectl cnpg destroy`) even though it kept the same<br />name, so containment can be lifted and the entry cleared. |  |  |  |
