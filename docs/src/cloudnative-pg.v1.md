...
| `dataChecksums` _boolean_ | Whether checksums on data pages should be enabled by initdb.<br />When `true`, `-k` is passed to initdb. When `false`, on PostgreSQL 18<br />and later `--no-data-checksums` is passed, since checksums are enabled<br />by default there. When unset, the initdb default applies: disabled<br />up to PostgreSQL 17, enabled from PostgreSQL 18. |  |  |  |
