...
| `automountServiceAccountToken` _boolean_ | AutomountServiceAccountToken is copied verbatim to the<br />`automountServiceAccountToken` field of the instance Pods, of the<br />pod template of the instance creation Jobs, and of the<br />ServiceAccount generated by the operator (existing ServiceAccounts<br />referenced via `serviceAccountName` are never modified).<br />Since the instance manager requires access to the Kubernetes API,<br />when this field is set to `false` the operator mounts an explicit<br />projected volume providing the ServiceAccount token, equivalent to<br />the one that would have been automounted.<br />When unset, the default Kubernetes behavior applies (the token is<br />automounted). |  |  |  |
