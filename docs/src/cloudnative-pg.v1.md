...
| `hostUsers` _boolean_ | Determines if the pod's processes run in the host's user namespace.<br />If set to `false`, a user namespace is created for the pod and all<br />the container processes run with a different UID/GID than on the host.<br />Requires Kubernetes 1.25+ with user namespace support enabled.<br />More info: https://kubernetes.io/docs/concepts/workloads/pods/user-namespaces/ |  |  |  |
