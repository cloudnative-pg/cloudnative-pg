...
#### Automatic mount of the ServiceAccount token
...
`ServiceAccount` itself. To comply with such policies, CloudNativePG always
sets `automountServiceAccountToken: false` on the instance Pods, on the pod
template of the Jobs that create the instances, and on the `ServiceAccount`
generated by the operator. This behavior is not configurable.

Since the instance manager requires access to the Kubernetes API server, the
operator adds to those Pods and Jobs an explicit projected volume named
`kube-api-access`, equivalent to the volume that Kubernetes would have
automounted, and mounts it at `/var/run/secrets/kubernetes.io/serviceaccount`
on every container except the initial bootstrap one (which only copies the
instance manager binary and never talks to the API server). As a result, the
workload keeps the same API server access it would have with the default
settings, while satisfying policies that check the
`automountServiceAccountToken` fields.

:::important
    Clusters created by an operator version predating this behavior have
    their instance Pods rolled once, the first time they are reconciled by an
    operator version that includes it, so that the new setting can be
    applied.
:::
...
:::note
    ...
    `automountServiceAccountToken: false` on the `ServiceAccount` itself is
    your responsibility in that case.
:::

:::note
    The operator only realigns the generated `ServiceAccount` when it
    reconciles the `Cluster` for some other reason. If something external
    flips `automountServiceAccountToken` back to `true` on that
    `ServiceAccount`, the drift is corrected on the next reconciliation
