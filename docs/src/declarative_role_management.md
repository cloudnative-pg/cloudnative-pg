...
The line is appended, since `~/.pgpass` usually holds the credentials of more
than one role: libpq reads the file top to bottom and uses the first line whose
host, port, database and user all match.
...
expires, and the expiration is also a column of `kubectl get databaserole`:
...
The annotation therefore disappears whether the request was honored or
discarded: what tells the two apart is the event the operator records on the
role, `PasswordRotated` or `PasswordRotationIgnored` (see
[Status of `DatabaseRole` resources](#status-of-databaserole-resources)).
...
and in the `Certificate Expiration` column of `kubectl get databaserole`; each
...
The operator also records what it did to the password of a role as Kubernetes
events, so that a rotation is visible without reading the logs of the operator
Pod:
...
| Reason | Type | When |
|---|---|---|
| `PasswordGenerated` | Normal | a password was generated into the Secret for the first time |
| `PasswordRotated` | Normal | a new password replaced the previous one, whether on its deadline or [on request](#manual-rotation) |
| `PasswordRotationIgnored` | Warning | a rotation was requested for a role whose password the operator does not generate |
| `PasswordGenerationSkipped` | Warning | no password could be generated, with the reason: a Secret the operator does not own, unsatisfiable criteria, a replica cluster |
| `ClientCertificateIssued` | Normal | a [client certificate](#client-certificate-authentication) was issued for the first time |
| `ClientCertificateRenewed` | Normal | the certificate was replaced, with the reason: an approaching expiration, a rotated client CA, a certificate that could not be read |
| `ClientCertificateIssuanceSkipped` | Warning | no certificate could be issued, with the reason: a Secret the operator does not own, a CA without a private key |

The two `Skipped` events are recorded when the reason changes, not on every
reconciliation: a cause that lasts, such as a replica cluster, is stated once
...
The `Password Expiration` column of `kubectl get databaserole` is the deadline
to watch, and a `PasswordGenerationSkipped` event on the role says what is in
the way. Pick a `renewBefore` that leaves room to notice a stalled rotation and
fix it before that deadline.
:::
