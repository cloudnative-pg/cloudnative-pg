...
condition appears once a password Secret is in use, whether supplied through
`passwordSecret` or [generated by the operator](#generating-passwords), and is
removed when the role no longer has one.
...
A `DatabaseRole` can also have the operator
[generate its password](#generating-passwords) instead of referring to an
existing Secret.
...
Instead of supplying a Secret, a `DatabaseRole` can ask the operator to
generate the password of the role and store it, through the `password` stanza:
