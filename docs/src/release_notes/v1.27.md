...
- Adopted the new format of `postgres-containers` and `postgis-containers`
  ...
  version to `18.0-system-trixie` (PostgreSQL 18 is supported).
...
- Added support for overriding the `auth_type`, `server_tls_sslmode`, and
  `client_tls_sslmode` settings, which were previously hardcoded. Default
  values remain consistent with the former behaviour but can now be customised
  when required.
...
- `cnpg` plugin:
   ...
   - Updated the Flexible I/O Tester (FIO) plugin image to
...
<!-- NO: 1.25 --><!-- NO: 1.26 -->
- Changed quorum-based failover to work correctly in clusters with only two
...
- Fixed a bug in major upgrades where a volume snapshot from a previous minor
  version could be incorrectly used to optimise replica creation.
  ([#8475](https://github.com/cloudnative-pg/cloudnative-pg/pull/8475))

- Changed `initdb` to wait for the application user secret before bootstrapping
...
- Fixed replica recovery source detection to consider WAL-archiver plugins,
  particularly when using `recovery_counter`.
  ([#8506](https://github.com/cloudnative-pg/cloudnative-pg/pull/8506))

- Changed the `Pooler` templating to correctly inherit settings for the
