...
- Remove `pg_database_size` from status probe. This was producing
  ...
  the current size of every database. The pg_database_size will now only be used by the kubectl status plugin to retrieve detailed information (#5689).
...
- Add support for specifying Docker image with `--image` flag in the pgadmin4
  plugin command, allowing users to define the Docker image for the pgAdmin4
  deployment (#5515).
...
- Prioritize full rollout over in-place upgrades, in cases where several
  conditions require pod rollout, and any one of these require full pod re-creation, preempting a second consecutive rollout (#5407).
