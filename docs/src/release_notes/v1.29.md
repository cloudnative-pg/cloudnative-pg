...
- Improved the `Pooler` CRD with support for granular configuration of TLS
  ...
  ([#9571](https://github.com/cloudnative-pg/cloudnative-pg/pull/9571))
...
- Enhanced the security and reliability of role management by verifying the
  ...
  ([#9971](https://github.com/cloudnative-pg/cloudnative-pg/pull/9971))
...
- Refined the `alpha.cnpg.io/unrecoverable` annotation logic to allow it to
  ...
  ([#9968](https://github.com/cloudnative-pg/cloudnative-pg/pull/9968))
...
- **Security Best Practices Integration**: Integrated the OpenSSF baseline
  ...
  ([#10054](https://github.com/cloudnative-pg/cloudnative-pg/pull/10054),
  [#10062](https://github.com/cloudnative-pg/cloudnative-pg/pull/10062))
...
- **SLSA Provenance and SBOMs**: Added SLSA (Supply-chain Levels for Software
  ...
  ([#10048](https://github.com/cloudnative-pg/cloudnative-pg/pull/10048),
  [#10074](https://github.com/cloudnative-pg/cloudnative-pg/pull/10074))

- Fixed a potential security risk where PostgreSQL could leak role passwords in
  the logs during specific reconciliation phases.
  ([#9950](https://github.com/cloudnative-pg/cloudnative-pg/pull/9950))

### Changes

- Updated the default PostgreSQL version to 18.3 (image `18.3-system-trixie`).
  ([#10090](https://github.com/cloudnative-pg/cloudnative-pg/pull/10090))
...
- Fixed an issue where replicas would get stuck in a `Pending` state if the
  ...
  `pg_basebackup`.
  ([#10192](https://github.com/cloudnative-pg/cloudnative-pg/pull/10192))
...
- Prevented the "supervised primary" rollout strategy from consuming all
  ...
  ([#9977](https://github.com/cloudnative-pg/cloudnative-pg/pull/9977))

- Fixed an issue where certain hot-standby parameter changes were not being
  correctly applied to replica clusters.
  ([#9952](https://github.com/cloudnative-pg/cloudnative-pg/pull/9952))
...
- Fixed a bug in the CNPG-I reconciler hook that could lead to skipping
  ...
  ([#9978](https://github.com/cloudnative-pg/cloudnative-pg/pull/9978))
...
- Fixed a deadlock scenario that occurred when attempting to resize a
  ...
  ([#9981](https://github.com/cloudnative-pg/cloudnative-pg/pull/9981))
...
- Refined the bootstrap recovery logic by adding a missing check for
  ...
  configuration types.
  ([#10268](https://github.com/cloudnative-pg/cloudnative-pg/pull/10268))

- Volume names for extensions and tablespaces are now prefixed to avoid naming
  collisions with standard cluster volumes.
  ([#9973](https://github.com/cloudnative-pg/cloudnative-pg/pull/9973))
...
- Removed unnecessary pod existence checks from the `FencingMetadataExecutor`
  ...
  ([#10035](https://github.com/cloudnative-pg/cloudnative-pg/pull/10035))

- `cnpg` plugin:

    - The cnpg plugin now correctly propagates ImagePullSecrets to the
      `pgbench`Job pod template.
      ([#10174](https://github.com/cloudnative-pg/cloudnative-pg/pull/10174),
