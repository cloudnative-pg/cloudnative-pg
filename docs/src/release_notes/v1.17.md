...
- **Separate volume for WAL files:** introduction of the optional `walStorage`
  section to separate WAL files (`pg_wal`) in a dedicated volume, separate from
  the `PGDATA` defined in the main and mandatory `storage` section (#513).
  Current limitations: `walStorage` can only be set at cluster creation, and
  cannot be added, or removed when the cluster is up and running.
  **This is the first implementation of additional volumes support, which will
  lead to native support for tablespaces.**
