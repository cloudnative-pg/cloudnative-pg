...
<!-- Verify, then either backport or add to 1.29.0
...
- `cnpg` plugin:
    ...
    - Updated the `status` command to explicitly display "Disabled" when the
      `skipWalArchiving` annotation is present on a cluster. This replaces
      confusing "starting up" or "unknown" states when WAL archiving is intentionally
      bypassed.
...
- Resolved a bug where replicas could enter a crash-loop by attempting to
  download timeline history files that belong to the future. This occurred when
  stale files remained in the WAL archive from a previous cluster life, and
  replicas would incorrectly try to fetch them during recovery.
  ([#9650](https://github.com/cloudnative-pg/cloudnative-pg/pull/9650)) <!-- 1.27 1.25 -->

- Fixed a race condition in `replica_cluster` setups during the transition of a
  designated primary. The fix ensures that the new primary is correctly
  recognized before the old one is demoted, preventing transient "no primary"
  states in the replica cluster.
...
- Fixed a validation gap in Azure object store configurations where the
  `storageAccount` was not strictly required when using explicit credentials
  (such as a storage key or SAS token). The operator now correctly enforces that
  a storage account name is provided in these cases and ensures that the
  `connectionString` is mutually exclusive with other authentication parameters,
  preventing invalid backup and recovery configurations.
...
- Ensured that `imagePullSecrets` are correctly added to the `ServiceAccount`
...
- Added a check to verify ownership before the operator deletes a `PodMonitor`.
  ...
  monitoring resources that happen to share a name with expected CNPG resources.
...
- Fixed a bug where `pg_stat_archiver` metrics would continue to report stale
  data on standby instances after a switchover. The exporter now intelligently
  skips these metrics on standbys, as PostgreSQL only provides valid archiver
  stats on the primary.
  ([#9411](https://github.com/cloudnative-pg/cloudnative-pg/pull/9411)) <!-- 1.27 1.25 -->

- Clarified the behavior of the `targetTime` field in the `Recovery`
  specification. The documentation now explicitly states that timestamps are
  interpreted in UTC and provides clear examples of supported ISO formats to
  prevent recovery failures due to timezone ambiguity.
  ([#8937](https://github.com/cloudnative-pg/cloudnative-pg/pull/8937)) <!-- 1.27 1.25 -->

- Switched to `Status().Patch` for updating backup status in
  `reconcileSnapshotBackup`, preventing potential data loss from "resource has
  been modified" errors during concurrent updates.
  ([#9551](https://github.com/cloudnative-pg/cloudnative-pg/pull/9551)) <!-- 1.27 1.25 -->

- Fixed the event reporting to use the correct pod name when a backup pod is
  not found, ensuring logs and events point to the right resource during
  troubleshooting.
  ([#9552](https://github.com/cloudnative-pg/cloudnative-pg/pull/9552)) <!-- 1.27 1.25 -->

- Switched `scheduledbackup` logic to use a label-based indexer instead of
  standard ownership lookups, improving performance for clusters with a very
...
- Ensured that errors encountered during the removal of finalizers on
  `Database` objects are properly bubbled up and logged.
