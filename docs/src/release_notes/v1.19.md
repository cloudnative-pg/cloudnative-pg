...
- Allow overriding the default backup target policy in the backup (#1602)
  Previously, all backups and schedule backups would use the cluster-level
  target policy.

Enhancements:

- Enable debug option for initdb job (#1503)
  Uses the cluster’s LogLevel field to pass the debug log option to the `initdb`
  executable.
- Support ipv6 and custom pg_hba for pgbouncer (#1395)
- Add new metrics for last available and failed backup (#1428)
  `cnp_collector_last_available_backup_timestamp` and
  `cnp_collector_last_failed_backup_timestamp`
- Add support for http json logs (#1442)
...
- Inject binaries for all architectures in the operator image (#1513)
