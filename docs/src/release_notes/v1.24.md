...
- Support the new metrics introduced in PgBouncer 1.23 in the `Pooler` metrics
  collector (#5044).
- Enhance the `cnpg` plugin `publication` and `subscription` subcommand
  to customize the remote database name (#5113)
- Add support for customizing the remote database name in the `publication`
  and `subscription` plugin subcommands. This enhancement provides greater
  flexibility when synchronizing data from an external cluster containing
  multiple databases (#5113).
