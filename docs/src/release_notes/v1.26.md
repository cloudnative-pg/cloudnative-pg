...
- Added admission warnings for potentially confusing storage configurations.
  Warnings are now issued when conflicting storage settings are specified
  (e.g., both `spec.storage.size` and `spec.storage.pvcTemplate.resources.requests.storage`),
  clarifying which value will be used. These checks apply to both `spec.storage` 
  and `spec.walStorage`.
  ([#8127](https://github.com/cloudnative-pg/cloudnative-pg/pull/8127))
