...
- Corrected replication lag comparison logic in custom health probes to accept
  equality, enabling probes to succeed when lag is exactly zero (#7442). <!-- no 1.25 1.24 1.22 -->
...
- Enhanced declarative major version upgrade logic by prioritizing the declared
  PostgreSQL version when using image catalogs. This change also replaces the
  deprecated `majorVersionUpgradeFromImage` field with the new `pgDataImageInfo`
  object in cluster status (#7387, #7403). <!-- no 1.25 1.24 1.22 -->
- `cnpg` plugin:
...
- Initiated deprecation of in-tree Barman Cloud support:
    ...
    - While support for in-tree object store backup and recovery remains
      available in this release, it is planned for removal in version 1.28.0.

    - *We strongly encourage users to begin planning the migration of their
      fleet to the new plugin-based approach. This release candidate provides
      an opportunity to test the new plugin workflow ahead of the final 1.26.0
