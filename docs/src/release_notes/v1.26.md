...
- Introduced a new field in the `status` of the cluster resource to track the
  latest known Pod IP, improving observability and integration with networking
  tools (#7546).
- Enhanced the liveness probe with an opt-in experimental feature to detect
  network isolation scenarios on the primary instance. This feature can be
  explicitly enabled using the `alpha.cnpg.io/livenessPinger` annotation (#7466).
...
- Updated the default PgBouncer version for new `Pooler` deployments to 1.24.1
  (#7399).
