...
The cluster's `affinity` section allows fine-tuning how all its pods' will be scheduled.

The operator by default sets pod anti-affinity to spread PostgreSQL pods,
based on the selected `topologyKey` (for example `kubernetes.io/hostname`
to have them spread among different nodes or `topology.kubernetes.io/zone`
for availability zones - previously it was `failure-domain.beta.kubernetes.io/zone`
but [it has been deprecated starting from Kubernetes v1.17](https://kubernetes.io/docs/reference/labels-annotations-taints/#failure-domainbetakubernetesiozone)).
The generated anti-affinity can be disabled by setting `EnablePodAntiAffinity`
to `false`.

Additionally, users can specify a list of custom pod affinity or
anti-affinity rules via the `additionalPodAntiAffinity` and `additionalPodAffinity`
configuration attributes. These rules will be added to the ones generated by the operator,
if enabled, or passed transparently otherwise. See [Kubernetes documentation](https://kubernetes.io/docs/concepts/scheduling-eviction/assign-pod-node/#inter-pod-affinity-and-anti-affinity)
for more examples.

!!! Note
    You have to pass to `additionalPodAntiAffinity` or `additionalPodAffinity` the
    whole content of `podAntiAffinity` or `podAffinity` that is expected by the Pod spec,
    please look at the following YAML as a matter of example of having only one instance of
    PostgreSQL running on every worker node, regardless which PostgreSQL cluster they belongs to:
...
Similarly, the operator supports transparently passing both node affinity/anti-affinity
through the `nodeSelector` configuration attribute, to be specified as [expected by Kubernetes](https://kubernetes.io/docs/concepts/scheduling-eviction/assign-pod-node/)
and tolerations as `tolerations`. These will be added for all the pods created by the
