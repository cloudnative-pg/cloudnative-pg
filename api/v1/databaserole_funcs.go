...
// password of this role. The password block defaults enabled to true when
// present, so an unset enabled field means enabled.
func (r *DatabaseRole) IsPasswordGenerationEnabled() bool {
	if r.Spec.Password == nil {
		return false
	}
	return ptr.Deref(r.Spec.Password.Enabled, true)
}
...
// this role, either the one generated by the operator or the one supplied
// through `passwordSecret`. It is empty when the role has no password Secret.
