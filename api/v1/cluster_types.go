...
type LivenessProbe struct {
	...
	// Configure the primary isolation check, which verifies whether a primary
	// is isolated from both the Kubernetes API server and its replicas and
	// shuts it down when it is, so that it stops accepting writes it can no
	// longer safely serve. The check is driven by the age of the primary lease,
	// so its reaction time is set by the `primaryLease` timings rather than by
	// the liveness probe settings this stanza lives under. Enabled by default.
	// +optional
...
type IsolationCheckConfiguration struct {
	// Whether primary isolation checking is enabled
	// +optional
	// +kubebuilder:default:=true
