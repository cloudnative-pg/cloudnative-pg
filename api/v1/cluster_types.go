...
type ClusterSpec struct {
	...
	// AutomountServiceAccountToken is copied verbatim to the
	// `automountServiceAccountToken` field of the instance Pods, of the
	// pod template of the instance creation Jobs, and of the
	// ServiceAccount generated by the operator (existing ServiceAccounts
	// referenced via `serviceAccountName` are never modified).
	// Since the instance manager requires access to the Kubernetes API,
	// when this field is set to `false` the operator mounts an explicit
	// projected volume providing the ServiceAccount token, equivalent to
	// the one that would have been automounted.
	// When unset, the default Kubernetes behavior applies (the token is
	// automounted).
	// +optional
