...
func (in *ClusterStatus) DeepCopyInto(out *ClusterStatus) {
	*out = *in
	...
	if in.RolePasswordStatus != nil {
		in, out := &in.RolePasswordStatus, &out.RolePasswordStatus
		*out = make(map[string]PasswordState, len(*in))
		for key, val := range *in {
			(*out)[key] = val
		}
	}
	if in.RoleConfigurationsRejected != nil {
		in, out := &in.RoleConfigurationsRejected, &out.RoleConfigurationsRejected
		*out = make(map[string][]string, len(*in))
		for key, val := range *in {
			var outVal []string
			if val == nil {
