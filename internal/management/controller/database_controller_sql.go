...
// publicRole is the PostgreSQL pseudo-role that represents every current and
// future role. It must be emitted verbatim as the `PUBLIC` keyword and must
// never be quoted as an identifier, otherwise it resolves to a non-existent
// role literally named "public".
...
func applyObjectPrivilege(
	...
	for _, grantee := range grantees {
		...
		case apiv1.GrantUsageSpecType:
			mutation := fmt.Sprintf("GRANT %s ON %s %s TO %s",
				privilege, objectType, sanitizedObject, sanitizedGrantee) //nolint:gosec
		...
		case apiv1.RevokeUsageSpecType:
			mutation := fmt.Sprintf("REVOKE %s ON %s %s FROM %s",
				privilege, objectType, sanitizedObject, sanitizedGrantee) //nolint:gosec
			if _, err := db.ExecContext(ctx, mutation); err != nil {
