...
var _ = Describe("Slot synchronization", Ordered, func() {
	...
	It("can create slots in local from those on primary", func(ctx SpecContext) {
		...
		mockPrimary.ExpectQuery("^SELECT (.+) FROM pg_replication_slots").
			WillReturnRows(sqlmock.NewRows([]string{"slot_name", "slot_type", "active", "restart_lsn", "holds_xmin"}).
		...
		mockLocal.ExpectQuery("^SELECT (.+) FROM pg_replication_slots").
			WillReturnRows(sqlmock.NewRows([]string{"slot_name", "slot_type", "active", "restart_lsn", "holds_xmin"}))
		...
		mockLocal.ExpectExec("SELECT pg_replication_slot_advance").
	...
	It("can drop slots in local that hold xmin", func(ctx SpecContext) {
		...
		mockLocal.ExpectExec("SELECT pg_replication_slot_advance").WithArgs(slotWithXmin, "0/301C4D8").
			WillReturnResult(sqlmock.NewResult(1, 1))
