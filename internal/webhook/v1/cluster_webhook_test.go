...
var _ = Describe("validateStandbyConnectionParameters", func() {
	clusterEmpty := &apiv1.Cluster{
		Spec: apiv1.ClusterSpec{
			PostgresConfiguration: apiv1.PostgresConfiguration{
				StandbyConnectionParameters: nil,
			},
		},
	}

	clusterOk := &apiv1.Cluster{
		Spec: apiv1.ClusterSpec{
			PostgresConfiguration: apiv1.PostgresConfiguration{
				StandbyConnectionParameters: map[string]string{
					"tcp_user_timeout": "5",
				},
			},
		},
	...
	v := &ClusterCustomValidator{}
	DescribeTable(
		"validateStandbyConnectionParameters",
		func(cluster *apiv1.Cluster, valid bool) {
			if valid {
				Expect(v.validateStandbyConnectionParameters(cluster)).To(BeEmpty())
			} else {
				Expect(v.validateStandbyConnectionParameters(cluster)).ToNot(BeEmpty())
			}
		},
		Entry("without any standby connection parameters", clusterEmpty, true),
		Entry("with allowed customizations", clusterOk, true),
		Entry("with customizations that are not allowed", clusterKo, false),
	)
})
