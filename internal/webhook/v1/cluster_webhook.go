...
func (v *ClusterCustomValidator) validateApplicationDatabase(database string, owner string, command string,
) field.ErrorList {
	var result field.ErrorList
	// If you specify the database name, then you need also to specify the
	// owner user and vice versa
		...
		result = append(
			...
			field.Invalid(
				...
				fmt.Sprintf("External cluster %v cannot be used for recovery: "+
					"barmanObjectStore configuration is missing", r.Spec.Bootstrap.Recovery.Source)))
	}
...
func (v *ClusterCustomValidator) validateReplicaModeChange(r, old *apiv1.Cluster) field.ErrorList {
	var result field.ErrorList
