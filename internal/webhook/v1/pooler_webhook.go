...
func (v *PoolerCustomValidator) getAdmissionWarnings(r *apiv1.Pooler) admission.Warnings {
	...
	if r.Spec.PgBouncer != nil &&
		...
		warns = append(
			warns,
			"The .spec.pgbouncer.authQuerySecret field has been deprecated and replaced by .spec.pgbouncer.serverTLSSercret")
	}
...
func (v *PoolerCustomValidator) validatePgBouncer(r *apiv1.Pooler) field.ErrorList {
	...
	userSpecifiedCustomServerTLSSecret := r.GetServerTLSSecretName() != ""

	switch {
	case userSpecifiedCustomServerTLSSecret && r.Spec.PgBouncer.AuthQuery == "":
		result = append(result,
			field.Invalid(
