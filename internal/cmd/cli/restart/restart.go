...
func restart(ctx context.Context, clusterName string) error {
	...
	err = cli.Client.Patch(ctx, clusterRestarted, client.MergeFrom(&cluster))
	if err != nil {
...
func instanceRestart(ctx context.Context, clusterName, node string) error {
	...
	if cluster.Status.CurrentPrimary == node {
		...
		var pod corev1.Pod
		err := cli.Client.Get(ctx, client.ObjectKey{Namespace: cli.Namespace, Name: node}, &pod)
		if err != nil {
			return fmt.Errorf("while getting POD %v: %w", node, err)
		}
		if err := cli.Client.Delete(ctx, &pod); err != nil {
