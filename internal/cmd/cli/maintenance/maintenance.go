...
func getClusters(ctx context.Context, allNamespaces bool) (apiv1.ClusterList, error) {
	...
	if !allNamespaces {
		opts = append(opts, client.InNamespace(cli.Namespace))
	}
	err := cli.Client.List(ctx, &clusterList, opts...)
...
func patchNodeMaintenanceWindow(
	...
	err := cli.Client.Patch(ctx, maintenanceCluster, client.MergeFrom(&cluster))
	if err != nil {
		return err
	}
	return nil
