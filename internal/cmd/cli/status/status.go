...
func Status(
	...
	err := cli.Client.Get(ctx, client.ObjectKey{Namespace: cli.Namespace, Name: clusterName}, &cluster)
	if err != nil {
		return fmt.Errorf("while trying to get cluster %s in namespace %s: %w",
			clusterName, cli.Namespace, err)
	}

	status := extractPostgresqlStatus(ctx, cluster)
	hibernated, _ := isHibernated(status)

	err = cli.Print(status, format, os.Stdout)
	if err != nil || format != cli.OutputFormatText {
		return err
	}
...
func extractPostgresqlStatus(ctx context.Context, cluster apiv1.Cluster) *PostgresqlStatus {
	...
	if err := cli.Client.List(
		ctx,
		&pdbl,
		client.InNamespace(cli.Namespace),
...
func (fullStatus *PostgresqlStatus) printPostgresConfiguration(
	...
	customConf, _, err := utils.ExecCommand(ctx, client, cli.Config, fullStatus.PrimaryPod,
		specs.PostgresContainerName,
		&timeout,
		"cat",
	...
	pgHBAConf, _, err := utils.ExecCommand(ctx, client, cli.Config, fullStatus.PrimaryPod,
		specs.PostgresContainerName,
...
func (fullStatus *PostgresqlStatus) getBarmanObject(barmanObjectName string) (*ObjectStore, error) {
	...
	dynamicClient := dynamic.NewForConfigOrDie(cli.Config)
	unstructuredObj, err := dynamicClient.Resource(objectStoreGVR).Namespace(cli.Namespace).Get(
