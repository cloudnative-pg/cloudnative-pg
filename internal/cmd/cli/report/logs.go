...
func streamClusterLogsToZip(
	...
	err = cli.Client.List(ctx, &podList, matchClusterName, client.InNamespace(namespace))
	if err != nil {
...
func streamClusterJobLogsToZip(ctx context.Context, clusterName, namespace string,
	...
	if err := cli.Client.List(ctx, &jobList, matchClusterName, client.InNamespace(namespace)); err != nil {
	...
	for _, job := range jobList.Items {
		...
		var podList corev1.PodList
		if err := cli.Client.List(ctx, &podList, matchJobName, client.InNamespace(namespace)); err != nil {
