...
func addContentToZip(c interface{}, name, folder string, format cli.OutputFormat, zipper *zip.Writer) error {
	...
	if err = cli.Print(c, format, writer); err != nil {
		return fmt.Errorf("could not print '%s': %w", fileName, err)
	}
	return nil
}

func addObjectsToZip(objects []namedObject, folder string, format cli.OutputFormat, zipper *zip.Writer) error {
	for _, obj := range objects {
		...
		if err = cli.Print(obj.Object, format, objF); err != nil {
			return fmt.Errorf("could not print '%s': %w", fileName, err)
		}
	}
	return nil
