...
func (cr clusterReport) writeToZip(zipper *zip.Writer, format cli.OutputFormat, folder string) error {
	objects := []struct {
		content interface{}
		name    string
	}{
...
func cluster(ctx context.Context, clusterName, namespace string, format cli.OutputFormat,
	file string, includeLogs, logTimeStamp bool, timestamp time.Time,
) error {
	var events corev1.EventList
	err := cli.Client.List(ctx, &events, client.InNamespace(namespace))
	if err != nil {
		return fmt.Errorf("could not get events: %w", err)
	}

	var cluster apiv1.Cluster
	err = cli.Client.Get(ctx,
	...
	err = cli.Client.List(ctx, &pods, matchClusterName, client.InNamespace(namespace))
	if err != nil {
		return fmt.Errorf("could not get cluster pods: %w", err)
	}

	var jobs batchv1.JobList
	err = cli.Client.List(ctx, &jobs, matchClusterName, client.InNamespace(namespace))
	if err != nil {
		return fmt.Errorf("could not get cluster jobs: %w", err)
	}

	var pvcs corev1.PersistentVolumeClaimList
	err = cli.Client.List(ctx, &pvcs, matchClusterName, client.InNamespace(namespace))
	if err != nil {
	...
	if includeLogs {
		logsZipper := func(zipper *zip.Writer, dirname string) error {
			return streamClusterLogsToZip(ctx, clusterName, cli.Namespace, dirname, logTimeStamp, zipper)
		}

		jobLogsZipper := func(zipper *zip.Writer, dirname string) error {
			return streamClusterJobLogsToZip(ctx, clusterName, cli.Namespace, dirname, logTimeStamp, zipper)
		}
