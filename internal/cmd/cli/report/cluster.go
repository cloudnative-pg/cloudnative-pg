...
func clusterCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Args:  cli.RequiresArguments(1),
		ValidArgsFunction: func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
			return cli.CompleteClusters(cmd.Context(), args, toComplete), cobra.ShellCompDirectiveNoFileComp
		},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return cluster(cmd.Context(), clusterName, cli.Namespace,
				cli.OutputFormat(output), file, includeLogs, logTimeStamp, now)
		},
	}
