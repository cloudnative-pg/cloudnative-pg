...
func generateClusterPV(
	...
	for _, ipvc := range input {
		...
		err := c.Get(context.Background(), ipvc.AsNamespacedName(), &pvc)
		Ω(err).ToNot(HaveOccurred())

		patchedPvc := pvc.DeepCopy()
		patchedPvc.Spec.VolumeName = pvName
		err = c.Patch(context.Background(), patchedPvc, client.MergeFrom(&pvc))
		Ω(err).ToNot(HaveOccurred())
		err = c.Get(context.Background(), ipvc.AsNamespacedName(), &pvc)
		Ω(err).ToNot(HaveOccurred())
		...
		Ω(err).ToNot(HaveOccurred())
		err = c.Get(context.Background(), types.NamespacedName{Name: pvName}, &pv)
		Ω(err).ToNot(HaveOccurred())
...
var _ = Describe("unit test for pvc remapper logic", func() {
	...
	When("remapping is required", func() {
		...
		It("should rename PVC's", func() {
			...
			ipvcs = generateClusterPV(env.client, cluster, ipvcs)
