...
func (r *ClusterReconciler) reconcile(ctx context.Context, cluster *apiv1.Cluster) (ctrl.Result, error) {
	...
	//
	// If this happens, after a configurable timeout, the Kubelet of the
	// underlying node will remove all its Pods.
	//
	// If the connectivity resumes before that the operator will find two
	// primaries: the previously faulting one and the new primary that has been
	// promoted. The operator should just wait for the Pods to get its current
	// role from auto-healing to proceed. Without this safety measure, the
	// operator would just fail back to the first primary of the list.
