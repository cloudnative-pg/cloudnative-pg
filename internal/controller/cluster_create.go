...
func (r *ClusterReconciler) createPrimaryInstance(
	...
	// This must happen before generateNodeSerial to avoid:
	// 1. Status being updated with a new node serial
	...
	if pluginClient := cnpgiClient.GetPluginClientFromContext(ctx); pluginClient != nil {
		...
		if err != nil {
			return ctrl.Result{}, fmt.Errorf("cannot create temporary pod spec for lifecycle hook: %w", err)
		}
		_, err = pluginClient.LifecycleHook(ctx, plugin.OperationVerbCreate, cluster, tempPod)
		if err != nil {
