...
func InstanceManagerCommand(args []string) {
	...
	case "run":
		...
		createPgPass(pwFile)
		createPostgresConfigFile(pgData, postgresConfig)
		createHBAFile(pgData, postgresHBARules)
		runSubCommand()
	case "status":
...
// password file
func createPgPass(pwFile string) {
	if pwFile != "" {
...
// createPostgresConfigFile install in the PgData the file with
// the PostgreSQL settings which have been generated by the operator
func createPostgresConfigFile(pgData, postgresConfig string) {
	if postgresConfig != "" && pgData != "" {
		log.Log.Info(
			"Installing PostgreSQL configuration",
			"pgData", pgData,
			"postgresConfig", postgresConfig)
		if err := postgres.InstallCustomConfigurationFile(pgData, postgresConfig); err != nil {
			log.Log.Error(err, "Cannot install custom configuration file",
				"postgresConfig", postgresConfig)
			os.Exit(1)
		}
	}
...
// createHBAFile  install in the PgData the file containing
// the host-based access rules which is being managed by the operator
func createHBAFile(pgData, postgresHBARules string) {
	if postgresHBARules != "" && pgData != "" {
		log.Log.Info(
			"Installing pg_hba.conf file",
			"pgData", pgData,
			"postgresHBARules", postgresHBARules)
		if err := postgres.InstallPgHBAFile(pgData, postgresHBARules); err != nil {
			log.Log.Error(err, "Cannot install the hba file",
				"postgresHBARules", postgresHBARules)
			os.Exit(1)
		}
	}
