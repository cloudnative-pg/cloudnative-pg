...
var _ = Describe("Service accounts", func() {
	...
	It("create a service account with the cluster name", func() {
		...
		Expect(sa.Annotations[resources.OperatorManagedSecretsAnnotationName]).To(Equal("null"))
	})
	...
	It("correctly create the annotation storing the secret names", func() {
		...
		Expect(err).ToNot(HaveOccurred())
		Expect(sa.Annotations[resources.OperatorManagedSecretsAnnotationName]).To(Equal(`["one","two"]`))
	})
