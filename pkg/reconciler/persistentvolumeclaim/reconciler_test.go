	...
	It("Reconcile existing resources shouldn't fail and "+
		...
		pvcs := corev1.PersistentVolumeClaimList{
			Items: []corev1.PersistentVolumeClaim{
				makePVC(clusterName, "1", "1", utils.PVCRolePgData, false),
				makePVC(clusterName, "2", "2", utils.PVCRolePgWal, false),      // role is out of sync with name
				makePVC(clusterName, "3-wal", "3", utils.PVCRolePgData, false), // role is out of sync with name
				makePVC(clusterName, "3", "3", utils.PVCRolePgData, false),
			},
		}
...
var _ = Describe("PVC reconciliation", func() {
	...
	It("will reconcile each PVC's pvc-role labels if there are no pods", func() {
		...
		pvc := makePVC(clusterName, "1", "1", utils.PVCRolePgData, false)
		pvc2 := makePVC(clusterName, "2", "2", utils.PVCRolePgWal, false)         // role is out of sync with name
		pvc3Wal := makePVC(clusterName, "3-wal", "3", utils.PVCRolePgData, false) // role is out of sync with name
		pvc3Data := makePVC(clusterName, "3", "3", "", false)
	...
	It("will reconcile each PVC's instance-relative labels by invoking the instance metadata reconciler", func() {
		...
		pvc := makePVC(clusterName, "1", "0", utils.PVCRolePgData, false)
		pvc2 := makePVC(clusterName, "2", "0", utils.PVCRolePgData, false)
		pvc3Wal := makePVC(clusterName, "3-wal", "0", utils.PVCRolePgWal, false)
		pvc3Data := makePVC(clusterName, "3", "0", utils.PVCRolePgData, false)
		pvcs := []corev1.PersistentVolumeClaim{
			pvc,
			pvc2,
			pvc3Wal,
