...
func (o *onlineExecutor) finalize(
	...
	statusBody, err := o.backupClient.StatusWithErrors(ctx, targetPod.Status.PodIP)
	if err != nil {
	...
	case webserver.Started:
		res, err := o.backupClient.Stop(ctx,
			targetPod.Status.PodIP,
			*webserver.NewStopBackupRequest(backup.Name))
		if err != nil {
...
func (o *onlineExecutor) prepare(
	...
	if status != nil && (backup.Name != status.BackupName || status.Phase == "") {
		...
		res, err := o.backupClient.Start(ctx, targetPod.Status.PodIP, req)
		if err != nil {
