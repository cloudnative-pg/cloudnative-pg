...
var _ = Describe("ResizeDecisionEngine", Label("resizing"), func() {
	...
	Describe("DetermineStrategy", func() {
		...
		Context("when Auto strategy is configured", func() {
			BeforeEach(func() {
				cluster.Spec.ResourceResizePolicy = &apiv1.ResourceResizePolicy{
			...
			It("should select InPlace for small CPU changes", func() {
				strategy, err := engine.DetermineStrategy()
				Expect(err).NotTo(HaveOccurred())
				Expect(strategy).To(Equal(apiv1.ResourceResizeStrategyInPlace))
			})

			It("should select RollingUpdate for large CPU changes", func() {
				// Increase CPU by more than 100%
				newSpec.Requests[corev1.ResourceCPU] = resource.MustParse("2500m")
				newSpec.Limits[corev1.ResourceCPU] = resource.MustParse("5000m")
	...
	Describe("GetResizeReason", func() {
		...
		It("should provide reason for InPlace strategy", func() {
			reason := engine.GetResizeReason(apiv1.ResourceResizeStrategyInPlace)
			Expect(reason).To(ContainSubstring("suitable for in-place resize"))
		})

		It("should provide reason for RollingUpdate due to memory changes", func() {
			newSpec.Requests[corev1.ResourceMemory] = resource.MustParse("3Gi")
			newSpec.Limits[corev1.ResourceMemory] = resource.MustParse("6Gi")
			reason := engine.GetResizeReason(apiv1.ResourceResizeStrategyRollingUpdate)
			Expect(reason).To(ContainSubstring("Memory changes not supported"))
		})

		It("should provide reason for RollingUpdate due to threshold exceeded", func() {
			cluster.Spec.ResourceResizePolicy.CPU.MaxIncreasePct = &maxDec25 // 25%
			reason := engine.GetResizeReason(apiv1.ResourceResizeStrategyRollingUpdate)
			Expect(reason).To(ContainSubstring("exceed configured thresholds"))
		})
	})

	Describe("selectAutoStrategy", func() {
		Context("with default thresholds", func() {
			BeforeEach(func() {
				cluster.Spec.ResourceResizePolicy = &apiv1.ResourceResizePolicy{
					Strategy: apiv1.ResourceResizeStrategyAuto,
					CPU: &apiv1.ContainerResizePolicy{
			...
			It("should select InPlace for moderate CPU changes", func() {
				strategy := engine.selectAutoStrategy()
				Expect(strategy).To(Equal(apiv1.ResourceResizeStrategyInPlace))
			})
		...
		Context("with custom thresholds", func() {
			BeforeEach(func() {
				cluster.Spec.ResourceResizePolicy = &apiv1.ResourceResizePolicy{
					...
					AutoStrategyThresholds: &apiv1.AutoStrategyThresholds{
						CPUIncreaseThreshold: 25, // Lower threshold
					},
				}
			})

			It("should select RollingUpdate when exceeding custom threshold", func() {
				// 50% change exceeds 25% threshold
				strategy := engine.selectAutoStrategy()
				Expect(strategy).To(Equal(apiv1.ResourceResizeStrategyRollingUpdate))
			})
		})
