...
var _ = Describe("postgresql.auto.conf pruning", func() {
	var pgData string

	readAutoConf := func() string {
	...
	It("removes operator-managed replication settings and keeps other entries", func() {
		autoConf := `# Do not edit this file manually!
# It will be overwritten by the ALTER SYSTEM command.
restore_command = '/controller/manager wal-restore %f %p'
primary_slot_name = '_cnpg_cluster_1'
primary_conninfo = 'host=cluster-rw user=streaming_replica application_name=cluster-1'
idle_in_transaction_session_timeout = '10min'
`
		Expect(fileutils.WriteStringToFile(filepath.Join(pgData, "postgresql.auto.conf"), autoConf)).To(BeTrue())

		changed, err := PruneManagedReplicationSettingsFromAutoConf(pgData)
		Expect(err).ToNot(HaveOccurred())
		Expect(changed).To(BeTrue())

		content := readAutoConf()
		Expect(content).ToNot(ContainSubstring("primary_slot_name"))
		Expect(content).ToNot(ContainSubstring("primary_conninfo"))
		Expect(content).ToNot(ContainSubstring("restore_command"))
		Expect(content).To(ContainSubstring("idle_in_transaction_session_timeout"))
	})

	It("is a no-op when postgresql.auto.conf is missing", func() {
		changed, err := PruneManagedReplicationSettingsFromAutoConf(pgData)
		Expect(err).ToNot(HaveOccurred())
		Expect(changed).To(BeFalse())
	})

	It("is a no-op when there is nothing to prune", func() {
		autoConf := "log_min_duration_statement = '1s'\n"
		Expect(fileutils.WriteStringToFile(filepath.Join(pgData, "postgresql.auto.conf"), autoConf)).To(BeTrue())
