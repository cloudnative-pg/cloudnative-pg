...
func (instance *Instance) GetStatus() (result *postgres.PostgresqlStatus, err error) {
	...
	// Read the running configuration hash with current_setting, not the on-disk
	// value from pg_show_all_file_settings(): this hash and PendingRestart below
	// must reflect the same reload, otherwise a probe could report the
	// configuration as current while its restart is still pending. missing_ok
	// yields an empty hash for a configuration not generated by CloudNativePG.
